...
def tab9(self):
    ...
    self.TAB9_root_title=tk.Label(self.TAB9,text="Tax Microsimulation Model",
             font = self.fontStyle_title)
    self.TAB9_root_title.place(relx = self.title_pos_x, rely = self.title_pos_y, anchor = "n")
    
    self.TAB9_root_title=tk.Label(self.TAB9,text="Dashboard - under preparation",
             font = self.fontStyle_sub_title)
    self.TAB9_root_title.place(relx = self.title_pos_x, rely = self.sub_title_pos_y, anchor = "n")


def read_data(self, filename):
    self.data = pd.read_csv('output of calc.csv')
    print(self.data)
